...
function! vimwiki#html#Wiki2HTML(path, wikifile) "{{{
  ...
  " modified by lw2aw (inspired by hotoo@github)
  if exists("*strftime")
    if !exists('g:vimiwiki_timestamp_format')
      let g:vimwiki_timestamp_format = '%Y-%m-%d %H:%M:%S'
    endif
    call map(html_lines, 'substitute(v:val, "%time_stamp%", "'. strftime(g:vimwiki_timestamp_format, getftime(a:wikifile)) .'", "g")')
  endif
  " end modification by lw2aw
  "
